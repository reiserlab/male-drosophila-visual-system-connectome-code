...
def region_boxes(vol):
    ...
    -------
    boxes : ndarray
        array of shape (N, 2, 3) where N is the number of unique label values in vol
        (including label 0).
        The min box coordinate for label i is given in entry [i, 0, :] and the max in 
        entry [i, 1, :].
    """
    ...
    boxes = np.zeros((int(vol.max())+1, 2, 3), dtype=int)
    boxes[:, 0, :] = vol.shape
...
def fetch_brain_voxels(
    ...
    Download optic lobe voxels from google cloud, store as gzip, and create bounding box
    file roi-bounding-boxes-nm.csv
    ...
    r = requests.get(brain_url, allow_redirects=True, timeout=10)
    ...
    bbox_df = pd.DataFrame({
        'roi': rois[:roi_max],
        'x0_nm': x0[:roi_max], 'y0_nm': y0[:roi_max], 'z0_nm': z0[:roi_max],
        'x1_nm': x1[:roi_max], 'y1_nm': y1[:roi_max], 'z1_nm': z1[:roi_max]
        }).set_index('roi')
...
def voxelize_col_and_lay(
  ...
  , layers=True
    ...
    """
    Create npy and gzip with neuroglancer voxel volume of column and/or layer rois for all 
    optic lobe neuropils (currently ME/LO/LOP).

    The files (named, e.g. 'ME_ZYX_columns' and 'ME_ZYX_layers') save a 3D array (zyx notation)
    with integers indicating the columns and layers (format for columns: hex1_id*100+hex2_id, 
    and layers: 1 to number of layers)
    ...
    brain_url = os.environ['BRAIN_VOLUME_URL']
    brain_f = brain_url.split('/')[-1]
    brain_file_gz = data_path / f"{brain_f}.gz"
    ...
    for roi_str in rois:
        ...
        roi_box_zyx_nm = bbox_df\
            .loc[roi_str, ['z0_nm', 'y0_nm', 'x0_nm', 'z1_nm', 'y1_nm', 'x1_nm']]\
            .values
        _, _, _, z_1, y_1, x_1 = roi_box_zyx_nm // (8*samp)
        roi_vol_512nm = vol_512nm[:z_1, :y_1, :x_1]
        zv, yv, xv = np.meshgrid(
            np.arange(0, z_1*samp, samp)
          , np.arange(0, y_1*samp, samp)
          , np.arange(0, x_1*samp, samp)
          , indexing='ij')
        roi_xyz = np.concatenate((
                xv.reshape((-1,1))
              , yv.reshape((-1,1))
              , zv.reshape((-1,1)))
          , axis=1)
        ...
        neuprint_id = np.where( bbox_df.index==roi_str )[0][0]+1
